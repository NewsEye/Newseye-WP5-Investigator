...
def ts_to_dist(ts, smoothing=None):
...
def find_group_kl(corpus, group, item="lemma", granularity="month", min_count=10):
    ts, ts_ipm = corpus.timeseries(
        item=item, granularity=granularity, min_count=min_count, word_list=group
    )
...
def find_group_outliers(
    corpus, group, item="lemma", granularity="month", min_count=10, weights=None
):
...
def normalized_entropy_for_aligned_ts_ipm(
    corpus, item="lemma", granularity="month", min_count=10, smoothing=None
):
