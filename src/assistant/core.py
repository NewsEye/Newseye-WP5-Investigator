...
class SystemCore(object):
    ...
    def run_query(self, username, query, switch_query=False, threaded=True, return_dict=True):
        ...
        # ToDo: If the query has been done already, get the result from the local database
        #  existing_results = self._PSQL_api.get_query_results(querylist)

        current_query_id = self._PSQL_api.get_current_query_id(username)

        # Todo: delay estimates, first create the query, THEN store it to the database
        default_result = {
            'message': 'Not yet ready',
        }

        new_queries = [Query(query=query, parent_id=current_query_id, username=username, result=default_result) for query in querylist]

        query_ids = self._PSQL_api.add_queries(new_queries)

        # Add the correct ids to new_queries
        for i, query in enumerate(new_queries):
            query['query_id'] = query_ids[i]
        ...
        if switch_query:
            self._PSQL_api.set_current_query(username, new_queries[0]['query_id'])
        if return_dict:
            return new_queries
        else:
            return query_ids
    ...
    def topic_analysis(self, username):
        ...
        pub_dates.sort()
        last_query = current_state.query
        ...
        for id in result_ids:
            query, query_results = itemgetter('query', 'result')(state_history.get(id))
            year = query['f[{}][]'.format(conf.PUB_YEAR_FACET)]
            total_hits = query_results['meta']['pages']['total_count']
            for item in query_results['included']:
                if item['query_id'] == conf.TOPIC_FACET and item['type'] == 'facet':
