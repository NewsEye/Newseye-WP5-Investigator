...
class SearchTaskList(Resource):
    ...
    def get(self):
        ...
        tasks = [
            ...
            for task in Task.query.filter_by(user_id=current_user.id, task_type="search").all()
        ]
        if len(tasks) == 1:
            tasks = tasks[0]
        return tasks
    ...
    post_parser.add_argument(
        "q", location="json", help="A search query. Search the document text fields by default.",
    )
