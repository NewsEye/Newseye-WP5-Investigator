...
class InvestigationPattern(object):
   ...
   """
    ...
    async def __call__(self):
        ...
        await self.runner.run_subtasks_and_update_results(
            subtasks, self.estimate_interestingness, reference={"pattern": type(self).__name__},
        )

        return subtasks

    async def run_prerequisite_utilities(self):
        self.prerequisite_tasks = self.runner.generate_investigation_tasks(
            self.prerequisite_utilities, search_query=self.search_query
        )
        await self.runner.run_subtasks_and_update_results(
            self.prerequisite_tasks,
            self.estimate_interestingness,
    ...
    async def generate_subtasks(self):
        return self.runner.generate_investigation_tasks(
            [(self.utility_name, self.parameters)], search_query=self.search_query
        )
...
class Facets(InvestigationPattern):
    ...
    async def generate_subtasks(self):
        target_utilities = [
            ("common_facet_values", {"facet_name": facet_name, "n": self.parameters["n"]},)
            for facet_name in self.parameters["facet_names"]
        ]
        # TODO: more than one prerequisite_task?
        return self.runner.generate_investigation_tasks(
            target_utilities,
            source_uuid=self.prerequisite_tasks[0].uuid,
    ...
    async def update_parameters(self):
        ...
        if len(language_list) > 1:
            current_app.logger.error(
                "More than one language in a corpus %s" % prerequisite_task.search_query
            )
        ...
        if len(available_names) == 1:
            ...
            current_app.logger.error(
                "More than one model for language %s: %s" % (lang, available_models)
            )
            raise NotImplementedError("More than one model")
        else:
            current_app.logger.error(
                "Cannot find model for language %s: %s" % (lang, available_models)
            )
            raise NotFound(
                "No trained topic models exist for the selected model type and language."
            )
