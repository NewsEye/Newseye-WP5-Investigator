...
class ExpandQuery(AnalysisUtility):
    ...
    async def make_result(self):
        ...
        queries = [{"lang":l,
                    "word":word,
                    "num_words":self.task.parameters["max_number"]}
                   for word in self.input_data["words"] if len(word)>3 for l in langs]

        results = await asyncio.gather(*[self.query_similar_words(query)
