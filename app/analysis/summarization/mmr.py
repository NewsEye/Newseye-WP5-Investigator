    ...
    for d in document_indices:
        for s in summary_indices:
            if not np.array_equal(sentences_emb[d], sentences_emb[s]):
                weights += cosine_similarity(sentences_emb[d].reshape(1,len(sentences_emb[d])), sentences_emb[s].reshape(1,len(sentences_emb[s])))[0,0]

    penalty = 0.0
    for s1 in summary_indices:
        for s2 in summary_indices:
            if not np.array_equal(sentences_emb[s1], sentences_emb[s2]):
                penalty += cosine_similarity(sentences_emb[s1].reshape(1,len(sentences_emb[s1])), sentences_emb[s2].reshape(1,len(sentences_emb[s2])))[0,0]
...
def maximal_marginal_relevance(sentences, sentences_emb, lambd=2, r=0.6, budget=400):
    ...
    while len(U) > 0:
        ...
        for l in U:
            v = mmr(sentences_emb, G+[l], document, lambd) - mmr(sentences_emb, G, document, lambd)
            vr = v/(len(sentences[l].split())**r)
            if vr > valuer:
                value = v
                valuer = vr
                sentence = l
        if sum([ len(sentences[i].split()) for i in G ]) + len(sentences[sentence].split()) <= budget and value > 0:
            G = G + [sentence]
        U.remove(sentence)
    # Best singleton
