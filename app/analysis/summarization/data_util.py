...
def tokenizer_sentences(text, nlp, language):
    if language != "en" and language != "fr":
        try:
            nlp.add_pipe(nlp.create_pipe('sentencizer')) # 'sentencizer' already exists in pipeline
        except ValueError:
            pass
    doc = nlp(text)
...
def similar_to_summary(sentences, candidate, summary, similarity_threshold):
    for s in summary:
        if cosine_similarity(sentences[candidate].reshape(1,len(sentences[candidate])), sentences[s].reshape(1,len(sentences[s])))[0,0] > similarity_threshold:
            return True
    return False
