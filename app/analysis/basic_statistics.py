...
class ExtractWords(AnalysisUtility):
    ...
    async def count(self, response, word2docid):
        for word_info in response["docs"]:
            ...
            word = [
                word_info[f]
                for f in ["text_tfr_siv", "text_tse_siv", "text_tde_siv", "text_tfi_siv",]
                if f in word_info
            ][0]
            docid = word_info["id"][
                :-9
            ]  # word id has a form docid_pageno_word_wordno, e.g. la_presse_12148-bpt6k477386c_3_word_1
    ...
    async def call(self, task):
        ...
        for (s, e) in make_batches(len(self.input_data)):
            ...
            if batch_count == 20:
                await asyncio.gather(
                    *[self.search(self.input_data[s:e], word2docid) for (s, e) in batches]
                )
                batch_count = 0
                batches = []
        # the last batch
        ...
        return {"result": result, "interestingness": 0}
...
class ComputeTfIdf(AnalysisUtility):
    ...
    async def call(self, task):
        ...
        return {
            "result": json.loads(
                df[["count", "ipm", "tfidf"]].to_json(orient="index", double_precision=6)
            ),
    ...
    async def query_data(self, task):
        ...
        # TODO: facet fields
        # not optimal
        total = await search_database(
            {"rows": 0, "q": " ".join(["%s : [* TO *]" % langf for langf in lang_fields]),}
        )
        ...
        for (s, e) in make_batches(len(word_list), batch_size=1000):
            ...
            if batch_count == 20:
                await asyncio.gather(
                    *[self.search(word_list[s:e], df, total, qf) for (s, e) in batches]
                )
                batch_count = 0
                batches = []
        # the last batch
