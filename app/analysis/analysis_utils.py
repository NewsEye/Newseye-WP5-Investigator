...
class AnalysisUtility(object):
    ...
    def set_defaults(self):
        if self.utility_parameters:
            self.parameter_defaults = {
                param["parameter_name"]: param["parameter_default"]
                for param in self.utility_parameters
            }
...
class CommonFacetValues(AnalysisUtility):
    ...
    async def call(self, task):
        ...
        return {"result": facet_list, "interestingness": interestingness}
...
class GenerateTimeSeries(AnalysisUtility):
    ...
    async def call(self, task):
        ...
        return {"result": analysis_results, "interestingness": 0}
...
class ExtractDocumentIds(AnalysisUtility):
    ...
    async def __call__(self, task):
        ...
        return {"result": document_ids, "interestingness": 0}
...
class LemmaFrequencyTimeseries(AnalysisUtility):
    ...
    async def __call__(self, task):
        filename, item, item_type = itemgetter("corpus_filename", "item", "item_type")(
            task.utility_parameters
        )
        ...
        return {
            "result": {"absolute_counts": ts, "relative_counts": ts_ipm},
            "interestingness": 0,
        }
...
class AnalyseLemmaFrequency(AnalysisUtility):
    ...
    async def __call__(self, task):
        filename, word, suffix = itemgetter("corpus_filename", "word", "suffix")(
            task.utility_parameters
        )
        corpus = load_corpus_from_pickle(filename)
        if corpus is None:
        ...
        return {
            "result": {"wfr": wfr, "ts": ts, "ts_ipm": ts_ipm, "spikes": spikes},
            "interestingness": 0,
        }
