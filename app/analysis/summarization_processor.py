...
class Summarization(AnalysisUtility):
    ...
    async def make_result(self):
        ...
        # -------- Preprocessing texts -------- #
        document = data_util.process_document(document=texts, nlp=nlp, language=language)
        document = [ " ".join([ w.text for w in nlp(s) ]) for d in document for s in d ]
        document = [ s for s in document if len(s.split()) >= self.task.parameters["minimal_sentence_length"] ] # remove short sentences
        ...
        document_embeddings = data_util.embeddings_representation(clean_document, 'fasttext', nlp=nlp, language=lang)

        # -------- Sentence embeddings -------- #
        document_embeddings = [ data_util.sentence_representation(sentence, type=self.task.parameters["type_sentence_representation"]) for sentence in document_embeddings ]
