...
class TaskPlanner(object):
    ...
    async def execute_user_task(self, task_uuids=None):
        ...
        # A quick and dirty fix. In the future we should be able to execute_and_store a list of tasks in parallel.
        for task in tasks:
    ...
    async def get_prerequisite_tasks(self, task):
        # TODO: Fix the task history to work in the new way (original task is the parent and everything generated by the planner are under it)
        ...
        if input_task_uuid:
            ...
            if not source_utilities:
                input_task = generate_tasks(queries=('search', search_parameters), user=self.user, parent_id=task.uuid, return_tasks=True)
            else:
                task_parameters = {'utility': source_utilities[0],
                                   'target_search': search_parameters}

                input_task = generate_tasks(user=task.user, queries=('analysis', task_parameters),
                                                                  parent_id=task.uuid, return_tasks=True)
