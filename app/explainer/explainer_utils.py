...
from app import db
from app.models import InvestigatorRun, InvestigatorAction, Explanation, Task, TaskExplanation
...
def find_object(Table, uuid):
    ...
    obj = Table.query.filter_by(
        uuid=uuid, user_id=current_user.id
    ).first()
    if obj is None:
        raise NotFound(
...
def make_task_explanation(args):
    ...
    if explanation:
        current_app.logger.info("Explanation for task %s exists, not generating" %task_uuid)
...
def make_run_explanation(args):
    ...
    if explanation:
        current_app.logger.info("Explanation for run %s exists, not generating" %run_uuid)
        current_app.logger.debug("Explanation: %s" % explanation.explanation_content)
    else:
...
def generate_explanation(run, explanation_language, explanation_format):
    ...
    explanation = request_explanation(data, explanation_format, explanation_language, run.uuid)
...
def generate_task_explanation(task, run, explanation_language, explanation_format):
    ...
    while(tasks_to_explain):
        ...
        task_dict = {"name":task.processor.name,
                     "uuid":str(uuid)}
        if task.parameters:
            task_dict["parameters"]=task.parameters
        
        payload_data.append({"id":task_action["id"],
    ...
    explanation = request_explanation(payload_data, explanation_format, explanation_language, run.uuid)
