...
class TestComparison(TestUtility):
    ...
    response1 = requests.request("POST", url=url, data=payload1, headers={'content-type': "application/json", **headers})
    response2 = requests.request("POST", url=url, data=payload2, headers={'content-type': "application/json", **headers})
